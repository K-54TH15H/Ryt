...
namespace RYT
{
	...
	void GPUContextManager::Upload(const RaytracingContext* hostContext)
	{
		...
		// Hittables
		AllocateAndCopy(&deviceInfo.hittables, hostContext->hittables, hostContext->hittableCapacity);
		...
		// BVH Nodes
		AllocateAndCopy(&deviceInfo.bvhNodes, hostContext->bvhNodes, hostContext->bvhNodeCapacity);
		...
		// Materials
		AllocateAndCopy(&deviceInfo.materials, hostContext->materials, hostContext->materialCapacity);
		...
		// Textures
		AllocateAndCopy(&deviceInfo.textures, hostContext->textures, hostContext->textureCapacity);
		...
		cudaMemcpy(deviceContext, &localContext, sizeof(RaytracingContext), cudaMemcpyHostToDevice);
	...
	template<typename T>
	void GPUContextManager::AllocateAndCopy(T** devicePtr, T* hostPtr, size_t count)
	{
	...
	void GPUContextManager::AllocateImages(const RaytracingContext* hostContext)
	{
		...
		for(size_t imageIndex = 0; imageIndex < hostContext->imageSize; imageIndex++)
		{
